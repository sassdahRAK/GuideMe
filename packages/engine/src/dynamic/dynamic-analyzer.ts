...
import { safeIdSelector, harvestInteractiveElements } from './dom-harvester.ts';
import { matchDomElementWithFuse, synthesizeGroundedTutorial } from './fuse-dom-matcher.ts';
import { GeminiDomAnalyzer } from './gemini-dom-analyzer.ts';
import { IntentResolver } from '../intent/intent-resolver.ts';
import { LocalFallbackReranker } from '../intent/llm-reranker.ts';
...
export class DynamicPageAnalyzer {
   ...
   */
  static analyzePage(doc: any, url: string = ''): any {
    ...
    const allInputs = /** @type {HTMLInputElement[]} */ (Array.from(doc.querySelectorAll('input, select, textarea'))).filter((el: any) => {
    ...
    const buttons = Array.from(doc.querySelectorAll('button, [role="button"], input[type="submit"], input[type="button"], a.btn, a[class*="button"]')).filter((btn: any) => {
    ...
    const passwordInputs = allInputs.filter((el: any) => el.type === 'password');
    const emailOrUserInputs = allInputs.filter((el: any) => {
      const type = (el.type || '').toLowerCase();
      const name = (el.name || '').toLowerCase();
    ...
    const searchInputs = allInputs.filter((el: any) => {
      const type = (el.type || '').toLowerCase();
      const name = (el.name || '').toLowerCase();
      const placeholder = (el.placeholder || '').toLowerCase();
    ...
    const addToCartButtons = buttons.filter((btn: any) => {
    ...
    const saveOrSubmitButtons = buttons.filter((btn: any) => {
      const text = (btn.textContent || '').toLowerCase();
   ...
   */
  static classifyPageType(metrics: Record<string, any>): string {
    const {
      hasPasswordInput,
   ...
   */
  static async generateDynamicTutorialAsync(doc: any, url: string = '', userPrompt: string | Record<string, any> = '', options: Record<string, any> = {}): Promise<any> {
    ...
    if (reranker && !(reranker instanceof LocalFallbackReranker)) {
      try {
        ...
        if (typeof console !== 'undefined' && console.warn) {
          console.warn('[DynamicPageAnalyzer] Intent resolver fallback:', err?.message || err);
        }
      }
    }
    ...
    if (typeof userPrompt === 'string' && userPrompt.trim() && !isJsonPrompt) {
      ...
      if (hasBackendUrl) {
        try {
          ...
          if (fetchFn) {
            ...
            const serializableCandidates = candidates.map(({ element: _el, ...rest }: any) => rest);
          ...
          if (typeof console !== 'undefined' && console.warn) {
            console.warn('[DynamicPageAnalyzer] Backend AI proxy analysis fallback:', err?.message || err);
          }
        }
      }
      ...
      if (hasGeminiKey) {
        try {
          ...
          if (typeof console !== 'undefined' && console.warn) {
            console.warn('[DynamicPageAnalyzer] Gemini DOM analysis fallback:', err?.message || err);
          }
        }
      }
    }
   ...
   * @returns {Object|null}
   */
  static extractIntentFromText(text: string): Record<string, any> | null {
   ...
   */
  static generateDynamicTutorial(doc: any, url: string = '', userPrompt: string | Record<string, any> = '', options: Record<string, any> = {}): any {
    ...
    if (searchKeywords.length > 0 || matchedIntents.length > 0) {
      ...
      const scoreAndAdd = (el: any, type: string) => {
      ...
      analysis.buttons.forEach((btn: any) => scoreAndAdd(btn, 'button'));
      analysis.allInputs.forEach((input: any) => scoreAndAdd(input, 'input'));
      ...
      linksAndActionables.forEach((link: any) => scoreAndAdd(link, 'button'));
      ...
      if (candidates.length > 0) {
        ...
        const findDeepAncestor = (node: any, levels: number = 4) => {
          if (!node) return null;
      ...
      if (topCandidates.length > 0) {
        ...
        const matchedSteps = topCandidates.map((cand: any, idx: number) => {
    ...
    switch (analysis.pageType) {
      ...
      case 'signupForm': {
        ...
        analysis.allInputs.slice(0, 4).forEach((input: any, idx: number) => {
      ...
      case 'settingsPage': {
        ...
        analysis.allInputs.slice(0, 3).forEach((ctrl: any, idx: number) => {
      ...
      default: {
        ...
        topButtons.forEach((btn: any, idx: number) => {
   ...
   * @private
   */
  static _safeQueryAll(doc: any, selector: string): any[] {
   ...
   * @private
   */
  static _queryAllDeep(root: any, selector: string): any[] {
   ...
   * @private
   */
  static _isInteractable(el: any, options: Record<string, any> = {}): boolean {
    if (!el) return false;
   ...
   */
  static dispatchHoverEvents(element: any): void {
   ...
   * @private
   */
  static _findParentHoverTrigger(el: any): any {
    ...
    while (current && depth < 6) {
      ...
      if (isMenuContainer) {
        ...
        if (parent) {
          ...
          if (parent.children) {
            const childTrigger = Array.from(parent.children).find((c: any) => c !== current && c !== el && c.matches?.('button, a, [role="button"], [aria-haspopup]'));
   ...
   * @private
   */
  static _resolveHoverFlyouts(doc: any, searchKeywords: string[] = [], matchedIntents: any[] = []): Map<any, any> {
   ...
   * @private
   */
  static _calculateContextScore(el: any, intentId: string | null, options: Record<string, any> = {}): number {
    let score = 0;
    let current = el;
    let depth = 0;
    let hasModal = false;
   ...
   * @private
   */
  static _extractExplicitSelectors(promptText: string, doc: any): any[] {
   ...
   * @private
   */
  static _buildTargetSelector(el: any, defaultCssFallback: string = '', options: Record<string, any> = {}): Record<string, any> {
   ...
   */
  static extractInteractiveDom(doc: any, maxElements: number = 80): any[] {
    return GeminiDomAnalyzer.extractInteractiveDom(doc, maxElements);
  }

  static async analyzeWithGemini(params: any): Promise<any> {
    return GeminiDomAnalyzer.analyzeWithGemini(params);
  }

  static async _generateFromIntentResolver(doc: any, url: string, userPrompt: any, reranker: any, options: any): Promise<any> {
   ...
   * @private
   */
  static _buildStepFromDesc(desc: any, userPrompt: any, idx: number): any {
    const tag = desc.tag || '';
