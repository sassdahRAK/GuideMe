...
export { safeIdSelector };

export interface ExtractedDomCandidate {
  index: number;
  tag: string;
  type?: string;
  id?: string;
...
export interface AnalyzeWithGeminiParams {
  ...
  model?: string;
  language?: string;
  fetchFn?: ((url: string | URL | Request, init?: RequestInit) => Promise<Response>) | null;
}
...
export class GeminiDomAnalyzer {
   ...
   */
  static isElementVisible(el: any): boolean {
   ...
   */
  static extractInteractiveDom(doc: any, maxElements: number = 80): ExtractedDomCandidate[] {
    ...
    const candidates: ExtractedDomCandidate[] = [];
  ...
  }: AnalyzeWithGeminiParams): Promise<TutorialDefinition> {
    if (!apiKey) {
...
Generate the interactive tutorial JSON now.`;
    ...
    let tutorial: TutorialDefinition;
    try {
