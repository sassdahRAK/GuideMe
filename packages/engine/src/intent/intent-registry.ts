import { LlmReranker, LocalFallbackReranker, BackendIntentApiClient, BaseIntentReranker } from './llm-reranker.ts';
 ...
 */
export const AI_PRESETS: Record<string, AiPresetConfig> = {
  openai: {
    provider: 'openai',
...
export type IntentRerankerFactoryFn = (config: Record<string, any>) => BaseIntentReranker;
...
export class IntentRegistry {
  private static _customFactories: Map<string, IntentRerankerFactoryFn> = new Map();

  /**
   * Register a custom re-ranker factory.
   */
  static register(name: string, factory: IntentRerankerFactoryFn): void {
   ...
   */
  static create(presetOrConfig: string | Record<string, any> = 'openai', overrides: Record<string, any> = {}): BaseIntentReranker {
    // 1. Custom factory
   ...
   */
  static fromEnv(env: Record<string, any> = {}): BaseIntentReranker {
