import { FuseFilter } from './fuse-filter.ts';
import { LocalFallbackReranker } from './llm-reranker.ts';
import type { BaseIntentReranker } from './llm-reranker.ts';

export interface IntentResolverConfig {
  reranker?: BaseIntentReranker;
  candidateLimit?: number;
}

export interface IntentResolveOptions {
  maxSteps?: number;
  heuristicScorer?: (desc: any) => number;
  [key: string]: any;
}
...
export class IntentResolver {
  public reranker: BaseIntentReranker;
  public candidateLimit: number;
   ...
   *
   * @param candidates - All extracted element descriptors from DOM
   * @param promptText - User goal or prompt
   * @param options - Resolution options
   * @returns Selected, ordered candidate descriptors
   */
  async resolve(candidates: any[], promptText: string, options: IntentResolveOptions = {}): Promise<any[]> {
    ...
    const idToDescMap = new Map<string, any>();
    ...
    if (this.reranker && !(this.reranker instanceof LocalFallbackReranker)) {
      try {
        ...
        console.warn('[IntentResolver] Stage 2 LLM re-ranking failed or timed out. Falling back to Stage 1 Fuse.js:', err?.message || err);
      }
    }
   ...
   */
  resolveSync(candidates: any[], promptText: string, options: IntentResolveOptions = {}): any[] {
    const maxSteps = options.maxSteps || 3;
    const cleanPrompt = (promptText || '').trim();
    const stage1Results = FuseFilter.filterCandidates(candidates, cleanPrompt, this.candidateLimit, {
      heuristicScorer: options.heuristicScorer,
   ...
   */
  private _enforceSequenceDiscipline(candidates: any[], promptText: string, maxSteps: number): any[] {
    const selected: any[] = [];
