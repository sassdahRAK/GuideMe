 ...
 * Caches Audio Blobs in memory and IndexedDB using SHA-256 / polynomial hashing of text + voice.
...
export interface AudioCacheKeyOptions {
  voice?: string;
  lang?: string;
  language?: string;
  rate?: number | string;
...
export class AudioCache {
  private _memoryCache: Map<string, AudioCacheEntry>;
   ...
   */
  async computeKey(text: string, options: AudioCacheKeyOptions = {}): Promise<string> {
   ...
   */
  async get(text: string, options: AudioCacheKeyOptions = {}): Promise<Blob | null> {
    if (!text) return null;
   ...
   */
  async set(text: string, options: AudioCacheKeyOptions | Blob = {}, maybeBlob?: Blob): Promise<void> {
    let actualOptions: AudioCacheKeyOptions = options as AudioCacheKeyOptions;
    let actualBlob: any = maybeBlob;

    // Handle 2-arg signature set(text, blob)
    if (!maybeBlob && options && (typeof (options as any).size === 'number' || (typeof Blob !== 'undefined' && options instanceof Blob))) {
      actualBlob = options;
      actualOptions = {};
    }
   ...
   */
  private _storeMemory(key: string, blob: Blob | any): void {
...
export const globalAudioCache: AudioCache = new AudioCache();
