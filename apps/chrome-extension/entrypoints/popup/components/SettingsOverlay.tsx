...
import { SPEAKER_OPTIONS } from '../constants.ts';
 ...
 */
function AccordionSection({ icon, label, isOpen, onToggle, children }: AccordionSectionProps) {
  return (
 ...
 */
function OptionRow({ leftIcon, label, active, onClick, disabled = false, showRadio = true, rightIcon }: OptionRowProps) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    if (typeof chrome !== 'undefined' && chrome.storage?.local) {
      chrome.storage.local.get(
        ...
        (res: Record<string, any>) => {
          if (res?.guideme_ai_provider) setAiProvider(String(res.guideme_ai_provider));
          if (res?.guideme_gemini_api_key) setGeminiApiKeyInput(String(res.guideme_gemini_api_key));
          if (res?.guideme_gemini_model) setGeminiModelInput(String(res.guideme_gemini_model));
        }
      );
    }
  }, []);
      ...
      <div className="flex-1 overflow-y-auto p-3 pb-8 flex flex-col gap-2 bg-white dark:bg-[#101018] overscroll-contain">
          ...
          {isAuthenticated && userProfile ? (
            <div className="p-3.5 flex flex-col gap-3">
              ...
              <button
                type="button"
                onClick={() => {
                  const siteUrl = String(import.meta.env.WXT_SITE_URL || 'http://localhost:3005');
        ...
        >
          {LANGUAGES_FULL.map((lang) => {
            const labelObj = lang.label as Record<string, string>;
            const displayLabel = typeof lang.label === 'object' ? (labelObj[currentLanguage] || labelObj.en) : String(lang.label);
            return (
              <OptionRow
                key={lang.code}
        ...
        >
          {SPEAKER_OPTIONS.map((s) => {
            const labelObj = s.label as Record<string, string>;
            const displayLabel = typeof s.label === 'object' ? (labelObj[currentLanguage] || labelObj.en) : String(s.label);
            return (
              <OptionRow
                key={s.id}
