...
export default function App(): React.ReactElement {
  ...
  const updateMessages = (newMessages: ChatMessageItem[]) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (stored.guideme_chat_messages && Array.isArray(stored.guideme_chat_messages) && stored.guideme_chat_messages.length > 0) {
          setMessages(stored.guideme_chat_messages);
        } else {
          const defaultGreeting: ChatMessageItem[] = [
            {
              role: 'assistant',
              content: INITIAL_GREETINGS[String(stored[STORAGE_KEY_LANG] || 'km')] || INITIAL_GREETINGS.km,
              time: nowTime(String(stored[STORAGE_KEY_LANG] || 'km')),
            },
          ];
    ...
    const storageListener = (changes: Record<string, chrome.storage.StorageChange>, areaName: string) => {
      if (areaName === 'local') {
        ...
        if (changes.guideme_chat_messages) {
          setMessages((changes.guideme_chat_messages.newValue as ChatMessageItem[]) || []);
        }
      }
    };
