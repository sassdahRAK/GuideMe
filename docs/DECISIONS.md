...
- **Decision:**
  ...
  2. **Provider-Agnostic Structured Intent Contract:** When actionable guidance is detected (`triggerGuide: true`), the AI (whether OpenRouter, Gemini, or fallback) outputs semantic search criteria (`targetQuery`, `action`, `role`, `category`) rather than guessing raw CSS selectors.
